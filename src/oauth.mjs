...
export function createOAuth(db) {
  ...
  async function token(body) {
    ...
    if(body.grant_type==='refresh_token') { const [r]=await db.execute("SELECT * FROM oauth_tokens WHERE token_hash=? AND token_type='refresh' AND revoked_at IS NULL AND expires_at>NOW()",[digest(body.refresh_token)]); if(!r[0]||r[0].client_id!==body.client_id) return fail('invalid_grant','Invalid refresh token'); await db.execute('UPDATE oauth_tokens SET revoked_at=NOW() WHERE family_id=? AND revoked_at IS NULL',[r[0].family_id]); return issue(r[0].client_id,r[0].user_id,r[0].scope,r[0].family_id); }
