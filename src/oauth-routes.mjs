...
export function mountOAuthRoutes(app, {db, oauth, rateLimit}) {
  ...
  app.post('/oauth/register', rateLimit({db, windowMs: 60000, max: 10}), async (req, res) => {
    ...
    console.info('oauth registration request', {host:req.get?.('host')||req.headers.host, forwardedProto:req.headers['x-forwarded-proto']||null, contentType:req.headers['content-type']||null, bodyKeys:Object.keys(body), clientNameLength:typeof body.client_name==='string'?body.client_name.length:null, redirectUriCount:Array.isArray(body.redirect_uris)?body.redirect_uris.length:null, redirectUris:Array.isArray(body.redirect_uris)?body.redirect_uris:null, authMethod:body.token_endpoint_auth_method||'none', grantTypes:body.grant_types||['authorization_code'], responseTypes:body.response_types||['code'], scope:body.scope||'funnel:read', resource:body.resource||body.resource_uri||'https://funnel.purinton.us/mcp'});
